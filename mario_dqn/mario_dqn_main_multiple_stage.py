...
from wrapper import BackgroundRemoveWrapper, MaxAndSkipWrapper, WarpFrameWrapper, ScaledFloatFrameWrapper, FrameStackWrapper, \
...
# mario环境
def wrapped_mario_env(version=0, action=7, obs=1,stage='1-1'):
...
def main(cfg, args, seed=0, max_env_step=int(5e6)):
    ...
    collector_env = SyncSubprocessEnvManager(
        env_fn=[partial(wrapped_mario_env, version=args.version, action=args.action, obs=args.obs, stage=stage) for stage in stages for _ in range(collector_env_num)], cfg=cfg.env.manager
    )
    # 评估性能的环境，使用并行环境管理器
    evaluator_env = SyncSubprocessEnvManager(
        env_fn=[partial(wrapped_mario_env, version=args.version, action=args.action, obs=args.obs, stage=stage) for stage in stages for _ in range(evaluator_env_num)], cfg=cfg.env.manager
    )
...
if __name__ == "__main__":
    ...
    mario_dqn_config.exp_name = 'exp_multiple/v'+str(args.version)+'_'+str(args.action)+'a_'+str(args.obs)+'f_seed'+str(args.seed)
