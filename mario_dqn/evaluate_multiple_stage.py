...
import gym
from wrapper import MaxAndSkipWrapper, WarpFrameWrapper, ScaledFloatFrameWrapper, FrameStackWrapper, BackgroundRemoveWrapper, \
...
def wrapped_mario_env(model, cam_video_path, version=0, action=2, obs=1, stage='1-1'):
    return DingEnvWrapper(
        JoypadSpace(gym_super_mario_bros.make("SuperMarioBros-"+stage+"-v"+str(version)), action_dict[int(action)]),
        cfg={
            'env_wrapper': [
                ...
                lambda env: RecordCAM(env, cam_model=model, video_folder=cam_video_path, stage_name=stage),
...
def evaluate(args, state_dict, seed, video_dir_path, eval_times, stage):
    # 加载配置
    ...
    # 生成环境
    env = wrapped_mario_env(model, video_dir_path, args.version, args.action, args.obs, stage)
    # 实例化DQN策略
...
if __name__ == "__main__":
    ...
    avg_reward = 0
    for stage in stages:
        avg_reward += evaluate(args, state_dict=state_dict, seed=args.seed, video_dir_path=video_dir_path, eval_times=1, stage=stage)
    print('All Eval is over! The performance of your RL policy is {}'.format(avg_reward / len(stages)))
